# G — Feature Catalog
## Objective
Produce deterministic schema-valid features with evidence and provenance.
## Inputs
Versioned parser reports.
## Outputs
Feature report schema `1.0.0`.
## Interfaces
`FeatureExtractor`, `PeFeatureExtractor`, `pasol-lab features`.
## Schemas
`schemas/feature-report-v1.schema.json`.
## Security requirements
No execution; preserve unknown/truncated/unsupported distinctions.
## Implementation checklist
- [x] SDK and PE extractor — `588b349`.
- [x] Runtime validation — `03d4f93`.
- [x] Golden reports and catalog-driven coverage â€” `TEST-EVIDENCE.md#2026-08-03-phase-g-feature-goldens`.
## Test checklist
- [x] Six-state serialization and malformed input.
- [x] Catalog presence, positive/negative baseline, and golden tests.
## Documentation checklist
- [~] Feature catalog and plan evidence.
## Acceptance criteria
All mandatory checklist items must be verified.
## Current status
Accepted at the Stage 2 foundation level; deterministic goldens and catalog-driven state evidence are verified.
## Completed commits
`00b24b8`, `588b349`, `03d4f93`.
## Remaining work
No mandatory Phase G implementation work remains; broader catalog expansion is future maintenance.
